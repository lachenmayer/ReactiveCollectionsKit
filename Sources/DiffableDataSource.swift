...
final class DiffableDataSource: UICollectionViewDiffableDataSource<AnyHashable, AnyHashable> {
    ...
    func reload(_ viewModel: CollectionViewModel) async {
    ...
    private func _visibleItemIdentifiers() -> Set<UniqueIdentifier> {
        ...
        let visibleSourceItemIdentifiers = visibleIndexPaths.compactMap {
            self.itemIdentifier(for: $0)
        }
    ...
    private func _reconfigureSupplementaryViewsIfNeeded(
        ...
        for sectionIndex in 0..<destination.sections.count {
            ...
            let allSourceSectionSupplementaryViews =
                sourceSection.allSupplementaryViewsByIdentifier()
    ...
    private func _visibleSectionIdentifiersFrom(destination: CollectionViewModel) -> Set<
        UniqueIdentifier
    > {
        ...
        let visibleSourceSectionIdentifiers = visibleSections.compactMap {
            self.sectionIdentifier(for: $0)
        }
    ...
    private func _reconfigureSupplementaryView(
        model: AnySupplementaryViewModel, item: Int, section: Int
    ) {
        let indexPath = IndexPath(item: item, section: section)
        if let view = self._collectionView.supplementaryView(
            forElementKind: model._kind, at: indexPath)
        {
